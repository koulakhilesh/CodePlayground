...
|------|----------------|-----------|------------|------:|---------|
| `HAS_CHAPTER` | Text → Chapter | — | Seed | 18 | Structural containment. |
| `HAS_VERSE` | Chapter → Verse | — | Extracted | 701 | Structural containment. |
| `NEXT` | Verse → Verse | — | Extracted | 683 | Reading order within a chapter. |
| `SPOKEN_BY` | Verse → Person | — | Extracted | 701 | Who speaks the verse (from the `"X said,"` prefix, inherited across runs). |
| `ADDRESSED_TO` | Verse → Person | — | Derived | 701 | Who the verse is addressed to (dialogue pairing). |
| `USES_EPITHET` | Verse → Epithet | — | Extracted | 32 | An honorific appears in the verse. |
| `EPITHET_OF` | Epithet → Person | — | Seed | 22 | Which person the honorific denotes. |
| `SET_IN` | Verse → Place | — | Seed | 1 | The setting reference. |
| `CHARIOTEER_OF` | Person → Person | — | Seed | 1 | Krishna is Arjuna's charioteer. |
| `NARRATES_TO` | Person → Person | — | Seed | 1 | Sanjaya narrates to Dhritarashtra. |
| `MENTIONS_TERM` | Verse → Term | `count` | Extracted | 6156 | English lemma occurs in the verse (with frequency). |
| `CONTAINS_TERM` | Verse → SanskritTerm | — | Extracted | 7995 | Sanskrit term occurs in the verse's Word Meanings. |
| `INCLUDES_TERM` | Theme → Term | — | Seed | 58 | The English lemmas that define a theme. |
| `MENTIONS_THEME` | Verse → Theme | `weight` | Derived | 1286 | Verse touches a theme; weight = summed term counts. |
| `INSTANCE_OF` | SanskritTerm → Concept | — | Derived | 139 | A Sanskrit term is an instance of a concept. |
| `EXPRESSES_CONCEPT` | Verse → Concept | `weight` | Derived | 841 | Verse expresses a concept; weight = distinct terms. |
| `ALIGNS_WITH` | Concept → Theme | — | Derived | 9 | A Sanskrit concept aligns with the English theme of the same name (the bridge between the two idea layers). |
| `MENTIONS_CHARACTER` | Verse → Character | — | Extracted | 310 | A character is named in the verse. |
| `NAMES_CONCH` | Verse → Conch | — | Extracted | 12 | A named conch appears in the verse. |
| `SOUNDS_CONCH` | Character → Conch | — | Extracted | 6 | The warrior who blows the conch (attested Ch.1 v15-16). |
| `DECLARES_VIBHUTI` | Verse → Vibhuti | — | Extracted | 13 | Verse is one of Krishna's "I am…" declarations. |
| `MANIFESTS_AS` | Character → Vibhuti | — | Extracted | 1 | Krishna is the one who manifests the glories. |
...
### 2. `Theme` vs `Concept` — resolved: kept both, bridged
